...
function createMatrix(n, m, minesData) {
  ...
  function printNumber(
    ...
    if (
      rowIndexPlus1 < rowArrayNew.length &&
      colIndexPlus1 < rowArrayNew[0].length
    ) {
  ...
  function selectBox() {
    ...
    function checkSpace(rowTargetId, colTargetId) {
      if (
        rowTargetId - 1 >= 0 &&
        colTargetId - 1 >= 0 &&
        rowTargetId + 1 < convert1DTo2DArrayData.length &&
        colTargetId + 1 < convert1DTo2DArrayData[0].length
      ) {
        convert1DTo2DArrayData[rowTargetId][colTargetId].classList.add(
          "fontSizeIncrement"
        );
        convert1DTo2DArrayData[rowTargetId - 1][colTargetId - 1].classList.add(
          "fontSizeIncrement"
        );
        convert1DTo2DArrayData[rowTargetId - 1][colTargetId].classList.add(
          "fontSizeIncrement"
        );
        convert1DTo2DArrayData[rowTargetId - 1][colTargetId + 1].classList.add(
          "fontSizeIncrement"
        );
        convert1DTo2DArrayData[rowTargetId][colTargetId - 1].classList.add(
          "fontSizeIncrement"
        );
        convert1DTo2DArrayData[rowTargetId][colTargetId + 1].classList.add(
          "fontSizeIncrement"
    ...
    function checkSpaceClick(rowTargetId, colTargetId) {
      if (
        convert1DTo2DArrayData[rowTargetId][colTargetId].innerHTML === "&nbsp;"
      ) {
        checkSpace(rowTargetId, colTargetId);
        if (rowTargetId - 1 >= 0 && colTargetId - 1 >= 0) {
          if (
            convert1DTo2DArrayData[rowTargetId - 1][colTargetId - 1]
              .innerHTML === "&nbsp;"
          ) {
        ...
        if (rowTargetId - 1 >= 0) {
          if (
            convert1DTo2DArrayData[rowTargetId - 1][colTargetId].innerHTML ===
            "&nbsp;"
          ) {
        ...
        if (colTargetId - 1 >= 0) {
          if (
            convert1DTo2DArrayData[rowTargetId][colTargetId - 1].innerHTML ===
            "&nbsp;"
          ) {
    ...
    matrix.addEventListener("click", (e) => {
      ...
      if (
        convert1DTo2DArrayData[rowTargetId][colTargetId].className.includes(
          "placeMinesData"
        )
      ) {
        for (let i = 0; i < convert1DTo2DIndex.length; i++) {
          if (
            convert1DTo2DArrayData[convert1DTo2DIndex[i].rowIndex][
              convert1DTo2DIndex[i].colIndex
            ].className.includes("placeMinesData")
          ) {
            convert1DTo2DArrayData[convert1DTo2DIndex[i].rowIndex][
              convert1DTo2DIndex[i].colIndex
            ].classList.add("findAllMineStyle");
